...
final class TasksViewController: UITableViewController {
    ...
    override func tableView(_ tableView: UITableView, trailingSwipeActionsConfigurationForRowAt indexPath: IndexPath) -> UISwipeActionsConfiguration? {
        ...
        let doneAction = UIContextualAction(style: .normal, title: doneTitle) { [unowned self] _, _, isDone in
            ...
            let pathForCurrentTask = IndexPath(row: currentTasks.count - 1, section: 0)
            let pathForCompletedTask = IndexPath(row: completedTasks.count - 1, section: 1)
            let destinationTaskPath = indexPath.section == 0 
            ? pathForCompletedTask : pathForCurrentTask
            
            tableView.moveRow(at: indexPath, to: destinationTaskPath)
            isDone(true)
        }
